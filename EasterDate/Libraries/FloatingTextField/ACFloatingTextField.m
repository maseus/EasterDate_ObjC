...
#define PLACEHOLDER_FONT [UIFont fontWithName: ROBOTO_REGULAR size: (IS_IPAD ? 15.0f : 12.0f)]
...
- (void) drawRect: (CGRect) rect
{
    ...
    [self updateTextField: CGRectMake (CGRectGetMinX(self.frame), CGRectGetMinY(self.frame), CGRectGetWidth(rect), CGRectGetHeight(rect) + (IS_IPAD ? 6.0f : 2.0f))];
}
...
- (CGRect) textRectForBounds: (CGRect) bounds
{
    ...
    return CGRectMake(4.0f, 4.0f, bounds.size.width, bounds.size.height + (IS_IPAD ? 6.0f : 2.0f));
}
...
- (void) addPlaceholderLabel
{
    ...
    if ([Utilities notNull: self.placeholder]) _labelPlaceholder.text = self.placeholder;
    NSString *placeHolderText = _labelPlaceholder.text;
    
    _labelPlaceholder = [[UILabel alloc]initWithFrame: CGRectMake(5.0f, 0.0f, self.frame.size.width - 5.0f, CGRectGetHeight(self.frame) + (IS_IPAD ? 6.0f : 2.0f))];
...
- (void) floatPlaceHolder: (BOOL) selected
{
    if (selected)
    {
        ...
        frame.size.height = (IS_IPAD ? 18.0f : 14.0f);
        CGRect bottomLineFrame = bottomLineView.frame;
        bottomLineFrame.origin.y = CGRectGetHeight(self.frame) - 2;
    ...
    else
    {
        ...
        CGRect frame = _labelPlaceholder.frame;
        frame.size.height = (IS_IPAD ? 16.0f : 12.0f);
        CGRect bottomLineFrame = bottomLineView.frame;
...
- (void) resignPlaceholder
{
    ...
    CGRect frame = CGRectMake(4.0f, 0.0f, self.frame.size.width - 5.0f, self.frame.size.height + (IS_IPAD ? 6.0f : 2.0f));
    CGRect bottomLineFrame = bottomLineView.frame;
    bottomLineFrame.origin.y = CGRectGetHeight(self.frame) - 1;
